...
- CubeMX 自動配腳通常是 `PA5 = SPI1_SCK`、`PA6 = SPI1_MISO`、`PA7 = SPI1_MOSI`——**這只是 MCU 邏輯腳名，不等於 Nucleo 接頭 D11 一定接到 PA7**（見下方 §「SB33 / SB35 與 MOSI 的關係」）。
- **NUCLEO-H743ZI2 實務建議**：先看板背 SB33/SB35，再決定 CubeMX 的 MOSI 要配 **PA7 還是 PB5**；多數板子（SB33 OFF、SB35 ON）應配 **PB5**（與 `RENESAS-F6222-Driver` demo 相同）。
- Parameter Settings:
  - `Data Size` = 8 Bits
...
| STM32 腳 | Arduino 名 | 訊號 | **CN7 針腳** | 經 SB33/SB35？ |
|---------|-----------|------|-------------|----------------|
| PA5 | D13 | SPI1_SCK | **pin 10** | 否（固定） |
| PA6 | D12 | SPI1_MISO | **pin 12** | 否（固定） |
| PA7 或 PB5 | D11 | SPI1_MOSI | **pin 14** | **是（二選一）** |
| PD14 | D10 | SPI1_CSB（軟體片選） | **pin 16** | 否（GPIO 固定） |

> GND 可取 CN7 **pin 8**（或同排任一 GND）。3.3 V 邏輯，勿當 5 V tolerance 用。

#### SB33 / SB35 與 MOSI 的關係（必讀，踩坑率極高）

**SB33、SB35 是 NUCLEO-H743ZI2 板上的焊橋（Solder Bridge）**，在 PCB 上（通常在板背或板面，標 `SB33` / `SB35` 的小焊盤）。它們決定 **CN7 pin 14（Arduino D11）這一根物理針，到底接到 MCU 的 PA7 還是 PB5**。

| 焊橋狀態 | 意思 |
|---------|------|
| **ON（接通）** | 兩焊盤之間有焊錫，線路導通 |
| **OFF（懸空/斷開）** | 兩焊盤未連接，該路徑斷開 |

**ST 官方對照（UM2407 Table 14，NUCLEO-H743ZI2 / MB1364）**：

| SB33 | SB35 | D11（CN7 pin 14）接到 MCU | CubeMX 應配 MOSI |
|------|------|---------------------------|------------------|
| **OFF** | **ON** | **PB5** | **PB5** |
| **ON** | **OFF** | **PA7** | **PA7** |

> ⚠️ **兩者不可同時 ON**，否則 PA7 與 PB5 可能短路。

**三層對應關係（搞懂就不會再量錯 MOSI）**：

```
韌體/CubeMX 設定的 MOSI 腳  ←必須一致→  板子 SB 選的路  ←必須一致→  示波器/杜邦線接的 CN7 針
     PA7 或 PB5                          PA7 或 PB5                         pin 14 (D11)
```

- **SCK（PA5）、MISO（PA6）、CSB（PD14）** 不經 SB33/SB35，所以「CLK/CSB 有波形、MOSI 沒有」幾乎一定是 **MOSI 腳名與 SB 配置不一致**。
- **CubeMX 預設 PA7** 只代表「SPI1 邏輯上用 PA7 當 MOSI」；若板子 SB 實際走 PB5，你在 D11 上永遠量不到 MOSI，但 F6222 demo（配 PB5）反而正常——這是實測常見現象，不是 HAL 壞掉。

**操作步驟（建議照做）**：

1. **停電**，用放大鏡看 Nucleo 板上 **SB33、SB35** 是否有焊錫連通。
2. 依上表查出 D11 接 **PA7 還是 PB5**。
3. 回 CubeMX Pinout：在晶片圖上把 SPI1_MOSI 設成**同一顆腳**（PA7 或 PB5），`GENERATE CODE`。
4. 用示波器量 **CN7 pin 14**，在 SPI 傳輸 command byte 時段看 MOSI（全 0x00 padding 時線可能一直低，別誤判成「沒輸出」）。

**本 repo 參考**：

| 專案 | MOSI 腳 | 適用的 SB 配置 |
|------|---------|----------------|
| `RENESAS-F6222-Driver/examples/stm32/f6222-spi-demo` | **PB5** | SB33 OFF、SB35 ON |
| `examples/stm32/stm32-h743-FPGA-middleware`（範本） | 目前 **PA7**（CubeMX 預設） | 僅適用 SB33 ON、SB35 OFF；若板子是 SB33 OFF / SB35 ON，請改 CubeMX 為 **PB5** |

**接線示意（MOSI 這一段）**：

```
STM32 PA7 ──[SB33]──┐
                    ├──► CN7 pin 14 (D11) ──► 你的 SPI slave MOSI
STM32 PB5 ──[SB35]──┘
         （同一時間只有一條路通）
```
...
左側 **Connectivity → USART3** → Mode `Asynchronous`，Parameter：`115200`、8N1、無硬體流控。

#### CubeMX 會自動產 vs 必須手補

| 項目 | CubeMX 自動？ | 在哪 |
|------|--------------|------|
| `MX_USART3_UART_Init()`、`huart3` | ✅ 自動 | `usart.c` / `usart.h` |
| `HAL_UART_MspInit()`（GPIO AF、時脈） | ✅ 自動（依 Pinout 腳位） | `usart.c` |
| `syscalls.c`（`_write` → `__io_putchar` 骨架） | ✅ 自動 | `Core/Src/syscalls.c` |
| **`__io_putchar()` 實作** | ❌ **無勾選項，必手寫** | `main.c` 或 `usart.c` 的 **`USER CODE`** 區塊 |
| **Nucleo VCP 腳 PD8/PD9** | ❌ 常配錯 | Pinout 手動點腳（見下） |

> CubeMX **沒有**「Generate printf retarget」之類的 Advanced Settings。ST 官方亦要求自行實作 `__io_putchar()`（見同 workspace 之 `RENESAS-F6222-Driver/examples/stm32/f6222-spi-demo` 的 `main.c`）。寫在 **`USER CODE BEGIN … END`** 內，重新 Generate Code **不會被蓋掉**。

#### 步驟 1：Pinout 設對 VCP 腳（NUCLEO-H743ZI2 + `/dev/ttyACM0`）

裸晶片建專案時，CubeMX 常預設 **PB10/PB11**；板載 ST-Link VCP 走 **PD8/PD9**（SB12/SB19 ON）。

1. 若 PB10/PB11 已設 USART3 → 點選 → **Reset_State**
2. 點 **PD8** → `USART3_TX`；點 **PD9** → `USART3_RX`
3. **Generate Code**

| 接法 | USART3 腳 | 主機看見 |
|------|-----------|----------|
| **板載 ST-Link VCP（預設）** | **PD8=TX, PD9=RX** | `/dev/ttyACM0` |
| 外接 USB-UART 轉接器 | PB10/PB11 等（依你的線） | 外接 COM 埠 |

#### 步驟 2：手補 `__io_putchar`（每次新建 CubeMX 專案都要做一次）

在 **`main.c`** 的 `USER CODE BEGIN 0`（或 **`usart.c`** 的 `USER CODE BEGIN 1`）加入：

```c
#include "usart.h"   /* 若寫在 main.c；usart.c 內則免 */
...
```c
MX_USART3_UART_Init();   /* 先 init UART */
/* USER CODE BEGIN 2 */
printf("hello\r\n");     /* 再 printf */
```
...
cat /dev/ttyACM0
# 或：minicom -D /dev/ttyACM0 -b 115200
```
...
| 現象 | 最可能原因 |
|------|------------|
| 完全無字 | 腳位仍是 PB10/PB11，或缺 `__io_putchar` |
| 有 `/dev/ttyACM0` 但空白 | 未 Rebuild/Flash；或 baud 不是 115200 |
| `HAL_UART_Transmit` 有波、`printf` 無 | 缺 `__io_putchar` 或沒 `#include <stdio.h>` |
| 只有 `\n` 換行怪 | 可選：`\n` 前加 `\r`（部分終端要 CRLF） |

本 repo 範本 [`examples/stm32/stm32-h743-FPGA-middleware/`](../examples/stm32/stm32-h743-FPGA-middleware/) 已含 PD8/PD9 + `__io_putchar`；**若你從 CubeMX 重產 `.ioc` 而刪了 USER CODE，記得把上面片段貼回。**
...
3. **不撞現有腳** — 避開 SWD(PA13/14)、HSE(PH0/1)、SPI1(PA5/PA6 + MOSI 的 PA7 或 PB5、PD14)、USART3 VCP(PD8/PD9)。
...
| SPI1 | Master, PA5=SCK / PA6=MISO / MOSI=**PA7 或 PB5**（須與 SB33/SB35 一致，見 §2.4）, 8-bit, NSS=Software；CN7：**pin10** SCK / **pin12** MISO / **pin14** MOSI（D11） |
| SB33/SB35 | OFF+ON→PB5；ON+OFF→PA7（UM2407 Table 14）。CLK/CSB 不受 SB 影響 |
