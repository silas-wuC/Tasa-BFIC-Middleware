...
> ⚠️ **NUCLEO-H743ZI2 用 `/dev/ttyACM0`（ST-Link Virtual COM Port）時，必須配 `PD8 = USART3_TX`、`PD9 = USART3_RX`。**
> ST-Link 預設焊橋（SB12/SB19 ON）把 VCP 接到 PD8/PD9，**不是 PB10/PB11**。CubeMX 新版常自動挑 PB10/PB11，燒進去後 `printf` 會完全沒輸出。
>
> 若要用外部 USB-UART 接 PB10/PB11，腳位可以不同；但走板載 ST-Link 序列埠就請改回 PD8/PD9。
>
> `printf` 還需在 `main.c` 的 `USER CODE BEGIN 0` 實作 `__io_putchar()`，呼叫 `HAL_UART_Transmit(&huart3, ...)`（見 F6222 demo）。
...
3. **不撞現有腳** — 避開 SWD(PA13/14)、HSE(PH0/1)、SPI1(PA5/PA6 + MOSI 的 PA7 或 PB5、PD14)、USART3(PB10/11)。
...
| USART3 | Async；**NUCLEO VCP 必須 PD8=TX / PD9=RX**（`/dev/ttyACM0`）；PB10/PB11 僅供外接 USB-UART |
